...
public class CellUtils extends MainController {
    ...
    public static int getCellWidth(String text){
        if (text == null) return ConstantController.TABLE_CELL_WIDTH;
        try {
            ...
            // Headless environment or graphics error fallback
            return text.length() * 8;
        }
    }
    
    public static int getCellWidth(mxCell jCell){
        if (jCell == null || jCell.getValue() == null) {
            return ConstantController.TABLE_CELL_WIDTH;
        }
        String text = jCell.getValue().toString();
